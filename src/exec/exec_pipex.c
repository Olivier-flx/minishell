...
/*   Updated: 2025/06/15 12:43:51 by ofilloux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	run_pipex(t_data *data, t_exe *exe, t_chunk *chunk, int i)
{
	...
	if (0 != execve_builtin_in_child(data, exe, chunk, i))
		execve(chunk->argv[0], chunk->argv, ft_env_to_array((*data).env)); // @optimize:  enviar una copia de list env en vez de la copia de env
	if (exe->total_cmd_count > 1)
	{
